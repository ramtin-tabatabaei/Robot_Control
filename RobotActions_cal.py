...
def run():
    ...
    if X>0.62:
        if Y>-0.18:
            X_3D = X_3D + 0.02  #Up Left
        else: 
            X_3D = X_3D + 0.02  #Up Right
    else:
        if Y<-0.18:
            X_3D = X_3D + 0.02  # Down Right
        else:
    ...
    if object_number == 1:
        ...
        if mode_camera > 0:
            if object_number==2 and Puzzle_number%2 == 1:
                yaw = -data[2][6]
            elif object_number==2 and Puzzle_number%2 == 0:
                yaw = data[2][6]
            elif object_number==3 and Puzzle_number%2 == 1:
                yaw = -data[1][6]
            elif object_number==3 and Puzzle_number%2 == 0:
                yaw = data[1][6]
    ...
    if failure_number !=2:
        ...
        #Step 7 going down
        goal_position = [Destination[0], Destination[1], -0.2485]  # Adjust as needed
        move_to_goal_position(goal_position, goal_orientation, 2)
        rospy.sleep(3)
        # publish_end_effector_pose()
        publish_state_end_effector([Participant_id, Puzzle_number, object_number, failure_number, Verbal_number, 7])
    ...
    move_arm_with_interp(up_joint_angles, 3)
    # if flag_head==0:
    ...
    if Verbal_number == 2:
        if failure_number == 2:
            ...
            #Step 3 Going Down
            goal_position = [Destination_failure[0], Destination_failure[1], -0.2485]  # Adjust as needed
            move_to_goal_position(goal_position, goal_orientation, 3)
            rospy.sleep(3)
